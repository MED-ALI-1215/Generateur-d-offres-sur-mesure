...
# NOUVELLE IMPORTATION POUR OPENAI
import openai
...
# NOUVELLE CONFIGURATION POUR OPENAI
os.environ["OPENAI_API_KEY"] = "sk-proj-..." # REMPLACEZ PAR VOTRE CLÉ API OPENAI
...
# NOUVELLE ROUTE POUR LA COMPARAISON DE TEXTE
...
def compare_texts():
    try:
        ...
        comparison_results = {
            "openai": {"similarity": -1, "error": None},
            "cohere": {"similarity": -1, "error": None}
        }
        
        # 1. Cohere Similarity Check
        if base_llm:
            try:
                cohere_response = base_llm.invoke(f"Compare the semantic similarity between these two texts and provide a score from 0.0 to 1.0. Text 1: '{text1}' Text 2: '{text2}'")
                
                match = re.search(r'\d+\.\d+', cohere_response.content)
                if match:
                    score = float(match.group(0))
                    comparison_results["cohere"]["similarity"] = score
                else:
                    raise ValueError("Could not parse similarity score from Cohere response.")
                    
            except Exception as e:
                comparison_results["cohere"]["error"] = str(e)
                print(f"Cohere comparison failed: {e}")
        else:
            comparison_results["cohere"]["error"] = "Cohere LLM not available."

        # 2. OpenAI Similarity Check
        if openai_client:
            try:
                # Utilisation d'une invite simple pour obtenir un score de similarité de gpt-3.5-turbo
                openai_response = openai_client.chat.completions.create(
                    model="gpt-3.5-turbo-0125",
                    messages=[
                        {"role": "system", "content": "You are a text comparison assistant. Your task is to analyze two provided texts and output a single floating-point number between 0.0 and 1.0 representing their semantic similarity. A score of 1.0 means they are identical in meaning, and 0.0 means they are completely unrelated. Respond with only the number, no other text."},
                ...
                score = float(openai_response.choices[0].message.content.strip())
                comparison_results["openai"]["similarity"] = score
            except Exception as e:
                comparison_results["openai"]["error"] = str(e)
                print(f"OpenAI comparison failed: {e}")
        else:
            comparison_results["openai"]["error"] = "OpenAI client not available."

        return jsonify({'comparison_results': comparison_results})

    except Exception as e:
        error_msg = f"Error in text comparison: {str(e)}"
        print(f"❌ {error_msg}")
        traceback.print_exc()
        return jsonify({'error': error_msg}), 500
...
if __name__ == '__main__':
    ...
    print("   POST /compare       - Compare two texts (NOUVEAU)")
