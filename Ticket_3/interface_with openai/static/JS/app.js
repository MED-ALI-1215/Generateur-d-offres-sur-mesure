...
class OfferGenerator {
    ...
    bindEvents() {
        ...
        // NOUVEL ÉVÉNEMENT POUR LE BOUTON "Compare"
        document.getElementById('compareBtn')?.addEventListener('click', () => this.compareOfferWithQuestion());
    ...
    // NOUVELLE FONCTION pour la comparaison
    async compareOfferWithQuestion() {
        const question = document.getElementById('questionInput').value.trim();
        const generatedOffer = this.currentGeneration?.final_result;

        if (!question || !generatedOffer || generatedOffer === "Erreur: Aucune génération d'offre n'a été produite.") {
            this.showError('You need a valid generated offer and a question to perform a comparison.');
            return;
        }

        this.showToast('Comparing offer with question...', 'info');
        ...
        try {
            ...
            this.showError(error.message || 'Comparison failed');
        }
    }

    // NOUVELLE FONCTION pour afficher les résultats de la comparaison
    showComparisonResults(result) {
        const openaiScore = result.comparison_results.openai.similarity.toFixed(4);
        const cohereScore = result.comparison_results.cohere.similarity.toFixed(4);
        
        const content = `
            <p><strong>OpenAI Similarity:</strong> ${openaiScore}</p>
            <p><strong>Cohere Similarity:</strong> ${cohereScore}</p>
            <p class="text-muted">A score closer to 1.0 indicates a higher similarity between the question and the generated offer.</p>
        `;

        this.showModal('Similarity Comparison', content);
