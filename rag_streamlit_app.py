...
class MultiModelGenerator:
    ...
    def __init__(self, cohere_api_key: str, github_api_key: str):
        self.cohere_api_key = cohere_api_key
        self.github_client = GitHubModelsClient(github_api_key)
...
Generate a professional, detailed offer in French. If you don't know specific details, use the context provided and general cybersecurity AI best practices."""
    ...
    def _estimate_cost(self, model_name: str, usage: dict) -> float:
        ...
        cost_per_1k_tokens = {
            ...
            "llama3": 0.0001,   # Often free or very cheap
        }
...
def initialize_rag_system(cohere_api_key, tavily_api_key):
    ...
    try:
        ...
        # In production, you would load from your actual PPTX files
        sample_documents = [
            Document(
                page_content="""
        ...
        Generate a professional, detailed cybersecurity AI offer in French with the following sections:
        1. RÉSUMÉ EXÉCUTIF
        ...
        hallucination_prompt = ChatPromptTemplate.from_messages([
            ...
            Your task is to determine if the generation is supported by the provided facts.
            - If the generation is supported by or grounded in the facts, respond with 'yes'
            - If the generation contains information not supported by the facts, respond with 'no'
            You must respond with ONLY 'yes' or 'no' - no other text or explanation."""),
            ("human", "Facts: {documents}\n\nGeneration: {generation}\n\nIs the generation grounded in the facts? Answer with 'yes' or 'no':")
        ])
        ...
        fallback_llm = ChatCohere(model_name="command-r", temperature=0).bind(preamble=fallback_preamble)
        ...
        generation_llm = ChatCohere(model_name="command-r", temperature=0).bind(preamble=generation_preamble)
        ...
        # Initialize metrics trackers (these will be reset for each query)
...
def generate_offer_with_multi_model(question: str, cohere_api_key: str, github_api_key: str = None, 
    ...
    try:
        ...
        if use_multi_model and github_api_key:
            ...
            result = generate_with_multi_model_evaluation(state, cohere_api_key, github_api_key)
...
def generate_with_multi_model_evaluation(state, cohere_api_key: str, github_api_key: str):
    ...
    try:
        ...
        multi_generator = MultiModelGenerator(cohere_api_key, github_api_key)
...
with st.sidebar:
    ...
    # ADD GITHUB MODELS CONFIGURATION HERE
    st.subheader("GitHub Models")
    github_api_key = st.text_input("GitHub Models API Key", type="password",
                                   help="Enter your GitHub Models API key for multi-model evaluation")
    
    enable_multi_model = st.checkbox("Enable Multi-Model Evaluation", value=False,
                                     help="Compare Cohere, Mistral, and Llama3 models")
    
    if enable_multi_model:
        st.info("Multi-model evaluation will compare responses from Cohere, Mistral, and Llama3")
            ...
            with st.spinner("Generating cybersecurity AI offer..."):
                try:
                    ...
                    result = generate_offer_with_multi_model(
                        question, 
                        cohere_api_key, 
                        github_api_key if enable_multi_model else None,
                        enable_multi_model
                    )
